...
use think\Request;
use app\admin\model\NewsCate as model;
/**
 * @title 新闻分类
 * @description ...
 */
class Newscate extends Paren
{   
    /**
     * @title 新闻分类列表
     * @description
     * @author 
     * @url /admin/newscate/lst
     * @method GET
     * @param name:name type:string require:1 default: other: desc:分类名称
     * name:pid type:int require:1 default: other: desc:父级分类
    ...
    //  self::adminAutho('test_add'); //权限验证，先注释
        ...
        $unique['unique']='news_cate,name';
        if(isset($param['id'])){
            $unique['unique']='news_cate,name,'.$param['id'];
        }
